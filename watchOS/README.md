...
圆形、表角和长方形。综合组件按时间段自动切换状态，小尺寸优先显示时间与
地点，搭配圆环或条形进度。名称和时间地点组件始终引用同一课程，日程概览
负责数量与分布。

倒计时使用“距上课”“距下课”；距上课 15 分钟内进入倒计时，下课不足一分钟
显示“即将下课”。实际末课结束后才展示明确标注日期的下一次安排，不在固定
晚间时刻覆盖还在进行的课程。综合组件的下一节预览绑定当前课程并自动到期，
不会影响分组件。缺少数据、缓存过期、已退出与确认无课分别显示。

Widget 只读取 App Group 的已校验缓存。时间线和 Watch 概览复用共享状态规则，
5×7 周矩阵在创建时预计算颜色，Canvas 不在绘制帧扫描课程。
