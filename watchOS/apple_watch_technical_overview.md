...
| `WidgetOnboardingView` | 五页阅读流程、轮播、表冠和结束页退出交互 |
...
| 5 | 黑色“教程结束 / 开始愉快的使用吧”，轻点结束页或从右向左滑动退出指南，返回概览 |
...
循环；第 3 页圆形有十个示例、长方形三个、表角一个综合课表课中示例。当前页
选中且 App 在前台时才播放；减少动态效果、VoiceOver 或第一页过渡期间暂停。
第五页不启动自动退出倒计时；轻点结束页或从右向左滑动才调用完成回调。离开或
进入后台会取消轮播与过渡任务，完成门保证同一次退出只调用一次结束回调。
...
2026-09-24 使用 Xcode 27.0（27A266a）、watchOS/iOS Simulator 27.0 SDK 完成
...
| Swift 日程、缓存与排序回归 | 124 项通过 |
| Swift 交互、生命周期与缓存回归 | 93 项通过 |
| Flutter Watch 快照回归 | 8 项通过 |
| 本地化审计脚本回归 | 8 项通过 |
| 签名脚本回归 | 6 项通过 |
| String Catalog 检查 | 224 条可翻译文案，简体源文案及繁体、英语完整 |
| Git 差异格式检查 | 通过 |
...
跳转、后台取消、第五页的轻点或从右向左滑动退出，以及减少动态效果和 VoiceOver 下的手动阅读。
