...
protocol WatchSyncSwiftApi {
  ...
  func clearSchedule(signedOut: Bool, completion: @escaping (Result<Bool, Error>) -> Void)
}
...
class WatchSyncSwiftApiSetup {
  ...
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: WatchSyncSwiftApi?, messageChannelSuffix: String = "") {
    ...
    if let api = api {
      clearScheduleChannel.setMessageHandler { message, reply in
        let args = message as! [Any?]
        let signedOutArg = args[0] as! Bool
        api.clearSchedule(signedOut: signedOutArg) { result in
          switch result {
          case .success(let res):
            reply(wrapResult(res))
          case .failure(let error):
