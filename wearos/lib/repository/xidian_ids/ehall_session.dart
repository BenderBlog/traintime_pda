...
class EhallSession extends IDSSession {
  ...
  Dio get dioEhallNoOfflineCheck =>
      dioNoOfflineCheck..options = BaseOptions(headers: refererHeader);

  Future<void> completeLoginRedirect(
    String location, {
    bool ignoreOffline = false,
  }) async {
    final initialDio = ignoreOffline ? dioNoOfflineCheck : dio;
    final redirectDio = ignoreOffline ? dioEhallNoOfflineCheck : dioEhall;
    var response = await initialDio.get(location);
    while (response.headers[HttpHeaders.locationHeader] != null) {
      location = response.headers[HttpHeaders.locationHeader]![0];
      log.info(
        "[ehall_session][completeLoginRedirect] "
        "Received location: $location",
      );
      response = await redirectDio.get(location);
    }
  }
  ...
  Future<void> loginEhall({
    ...
    await completeLoginRedirect(location);
  }
