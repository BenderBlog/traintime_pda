...
class IDSSession extends NetworkSession {
  ...
  static const _goLoginPasswordPrefix =
      '................................................................';
  static const _goLoginPasswordIv = '................';
  ...
  Future<bool> _hasCastgcCookie() async {
    final cookies = await cookieJar.loadForRequest(
      Uri.parse('https://ids.xidian.edu.cn/authserver/login'),
    );
    return cookies.any((cookie) => cookie.name == 'CASTGC');
  }

  /// Get base64 encoded data. Which is aes encrypted [toEnc] encoded string using [key].
  /// Matches the Go ids login payload: 64 fixed prefix bytes, AES-CBC,
  /// PKCS7 padding, and the fixed `................` IV.
  static String aesEncrypt(String toEnc, String key) {
    final crypt = encrypt.AES(
      encrypt.Key.fromUtf8(key),
      mode: encrypt.AESMode.cbc,
    );
    return encrypt.Encrypter(crypt)
        .encrypt(
          '$_goLoginPasswordPrefix$toEnc',
          iv: encrypt.IV.fromUtf8(_goLoginPasswordIv),
        )
        .base64;
  }

  static Map<String, dynamic> buildUsernameLoginPayloadForTesting({
    required String username,
    required String password,
    required String salt,
    required String execution,
  }) => _buildUsernameLoginPayload(
    username: username,
    password: password,
    salt: salt,
    execution: execution,
  ...
  static Map<String, dynamic> _buildUsernameLoginPayload({
    required String username,
    required String password,
    required String salt,
    required String execution,
  ...
  Future<String> login({
    ...
    final queryParameters = <String, String>{'type': 'userNameLogin'};
    if (target != null) {
    ...
    final execution = form
        .firstWhere(
          (element) =>
              element.attributes["name"] == "execution" ||
              element.id == "execution",
        )
        .attributes["value"]!;
    final head = _buildUsernameLoginPayload(
      username: username,
      password: password,
      salt: keys,
      execution: execution,
    ...
    try {
      ...
      final location = data.headers[HttpHeaders.locationHeader]?.first;
      if (location != null &&
          (data.statusCode == 301 ||
              data.statusCode == 302 ||
              await _hasCastgcCookie())) {
        /// Post login progress.
        if (onResponse != null) {
