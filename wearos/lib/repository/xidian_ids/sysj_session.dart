...
Future<FetchResult<List<ExperimentData>>> getOtherExperimentData() async {
  try {
    ...
    log.handle(e, s, "[SysjSession][getOtherExperimentData] Have issue");
...
class SysjSession extends IDSSession {
  ...
  static (DateTime, List<ExperimentData>)? getCache() {
    ...
    try {
      ...
      List<ExperimentData> otherData = List<ExperimentData>.generate(
        toDecode.length,
        (index) => ExperimentData.fromJson(toDecode[index]),
      );

      DateTime lastUpdateTime = otherExperimentCacheFile.lastModifiedSync();
      return (lastUpdateTime, otherData);
    } catch (e, s) {
      log.handle(e, s);
      log.warning(
        "[SysjSession][getCache] "
        "Failed to parse other experiment cache, will refresh.",
      );
      return null;
    }
  }
