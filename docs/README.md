...
如果是正式版软件
 ...
 - 如果是安卓系统，请去本软件官网（目前是 legacy.superbart.top/xdyou.html) 或者本项目的 Github Release (https://github.com/BenderBlog/traintime_pda/releases) 下载。
 - 如果是桌面端，请去本项目的 Github Release (https://github.com/BenderBlog/traintime_pda/releases) 下载。
 
如果是测试版软件

 - 如果是苹果系统，请加入 Testflight 测试。链接在此：https://testflight.apple.com/join/pLKe5B4q
...
如果您有博客，且博客支持 RSS 输出，则可以加入 XDU Planet。如需加入，请在这个文件（https://github.com/xdlinux/planet/blob/deploy/config.yml）下面添加您博客的 RSS 输出地址。
...
本软件涉及到的数据结构：[点击此处访问](data_structure.md)
