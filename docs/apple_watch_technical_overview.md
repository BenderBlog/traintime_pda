...
最多展示当前项与下一项，无当前项时仅显示下一项，不再重复显示卡片的上下课
时间。教师或考试备注与位置同行，同一天的日期也不在每张卡片上重复。
...
保留卡片内边距。长方形时间以单行范围显示，不再显示“上课／下课”标签行；
时刻与位置/教师行统一使用 16 pt 常规字重和主前景色，使两者保持相同信息权重；
标题为 12 pt，起止语义保留在辅助功能标签中。
所有尺寸都不显示倒计时。

圆形的综合课表与时间地点组件采用系统 `accessoryCircular` 开口圆环和进度圆点：
顶部为“下课”，中央为下课时间，底部开口放地点。课前隐藏圆环，按“上课、时间、
地点”三行显示；跨日提示并入第一行。表角上课中在外侧显示“精简位置 · 课程名”，
内侧为系统弧形进度；课前外侧只显示 `08:30`、`明天08:30` 或 `9/9 08:30`，
内侧改为下一节的“精简位置 · 课程名”。位置和课程名采用 12 pt 粗体，位置排在
...
Gauge 都继承该前景色，不再为课程进度环设置独立课程色。

只有焦点课程正在上课时才显示课程进度。没有 60 分钟档、首课等待档或固定晚间
切换。日程概览的 `summaryDate` 始终为当前日期，当天结束后保留“今日已下课”，
下一次的日期与时刻作为次要信息。本学期结束显示“本学期结束，开心玩耍吧！”。
...
不再保留按课程 ID 延迟定位的状态。切换箭头仍通过 App Intent 在组件内切换课程。
小组件入口暂停本次教学，不将教学永久标记为完成。

Timeline 节点覆盖 15 分钟状态阈值、上课、下课、自然日和缓存失效边界；只在
上课期间每五分钟增加进度刷新节点。进度均采用有限数值：圆形为系统 Gauge，
长方形由 Capsule 绘制，表角将 Gauge 放入 `widgetLabel`，外侧文字使用
`widgetCurvesContent`，由 WidgetKit 沿表盘边缘排布。不使用计时 ProgressView，避免 watchOS 26.5 模拟器 WidgetRenderer 的
`view origin is invalid: (nan, -1.0)` 布局断言。下一节预览不显示当前课程的进度。
无课、没有完整覆盖、过期、未同步及退出登录分别呈现。小组件已移除节次色带和
5×7 周矩阵；今日与本周统计分别要求完整覆盖，不展示部分同步数据的误导性总数。
