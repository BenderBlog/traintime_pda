...
class _DiningHallWindowState extends State<DiningHallWindow> {
              ...
              builder: (BuildContext context, AsyncSnapshot snapshot) {
                if (snapshot.connectionState == ConnectionState.done) {
                  if (snapshot.hasError) {
                    return Center(child: Text("坏事: ${snapshot.error} + ${snapshot.data}"));
                  } else {
                    return ListView(
                      children: [
                        for (var i in snapshot.data)
                          CafeteriaCard(toUse: i),
                      ],
                    );
                  }
                } else {
  ...
  Future<List<WindowInformation>> _get(bool isForceUpdate) async =>
      getCafeteriaData(toFind: toSearch, where: goToWhere, isForceUpdate: isForceUpdate);
            ...
            Row(
              ...
              children: [
                Text(
                  ...
                  style: TextStyle(
                    decoration: !toUse.status ? TextDecoration.lineThrough : TextDecoration.none,
                  ),
                ),
                Row(
                  children: [
