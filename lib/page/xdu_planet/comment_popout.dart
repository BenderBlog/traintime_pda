...
class CommentPopout extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: Text(context.t.xduPlanet.commentTitle),
      content: TextField(
        controller: _controller,
        maxLines: 5,
        decoration: InputDecoration(
          hintText: replyTo == null
              ? context.t.xduPlanet.hintSendComment
              : context.t.xduPlanet.reply(reply_to: replyTo!.ID.toString(), content: replyTo!.content),
        ),
      ),
      actions: [
        TextButton(
        ...
        TextButton(
          child: Text(context.t.xduPlanet.send),
          onPressed: () async {
            if (_controller.text.isEmpty) {
              showToast(
                context: context,
                msg: context.t.xduPlanet.emptySend,
              );
              return;
            }
            var pd = ProgressDialog(context: context);
            pd.show(msg: context.t.xduPlanet.sending);
