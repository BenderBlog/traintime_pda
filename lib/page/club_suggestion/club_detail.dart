  ...
  Widget build(BuildContext context) {
    return widget.code.isEmpty
        ? Scaffold(
            appBar: AppBar(
              title: Text(
                context.t.clubPromotion.wrongParam,
              ),
            ),
            body: Text(
              context.t.clubPromotion.noGroupInfo,
            ).center(),
          )
            ...
            builder: (context, snapshot) {
              if (snapshot.connectionState != ConnectionState.done) {
                return Scaffold(
                  appBar: AppBar(
                    title: Text(
                      context.t.clubPromotion.loading,
                    ),
                  ),
              ...
              if (snapshot.hasError) {
                return Scaffold(
                  appBar: AppBar(
                    title: Text(
                      context.t.clubPromotion.errorOutside,
                    ),
                  ),
                  body: ReloadWidget(
                    function: () => setState(() {
...
class _ClubDetailPageState extends State<ClubDetailPage> {
  ...
  Future<void> copyToClipboard() async {
    ...
    if (mounted) {
      showToast(
        context: context,
        msg: context.t.clubPromotion.qqCopied,
      );
    }
  }

  Future<void> openInviteLink() async {
                          ...
                          try {
                            return ConstrainedBox(
                              ...
                              child: Padding(
                                ...
                                child: HtmlWidget(
                                  snapshot.data ??
                                      '''<p>${context.t.clubPromotion.loadingProblem}</p>''',
                                ),
                              ),
                            );
                          } catch (e) {
