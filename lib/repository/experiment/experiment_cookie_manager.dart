...
// From: https://github.com/dart-lang/sdk/issues/42902
// It said we have nothing but re-implement the entire cookie in order
// to suit the 2008 GBK code.
/*
import 'dart:io';
...
class ExperimentCookieManager extends CookieManager {
  ExperimentCookieManager(CookieJar cookieJar) : super(cookieJar);
  ...
  Future<void> _saveCookies(Response response) async {
    ...
    final List<ExperimentCookie> cookies = setCookies
        .map((str) => str.split(_setCookieReg))
        .expand((cookie) => cookie)
        .where((cookie) => cookie.isNotEmpty)
        .map((str) => ExperimentCookie.fromSetCookieValue(str))
        .toList();
...
class ExperimentCookie implements Cookie {
  String _name;
  String _value;
  @override
  DateTime? expires;
  ...
  ExperimentCookie(String name, String value)
  ...
  ExperimentCookie.fromSetCookieValue(String value)
      : _name = "",
        _value = "" {
  ...
  static String _validateValue(String newValue) {
    ...
    for (int i = start; i < end; i++) {
      ...
      if (codeUnit < 0x20 || [0x22, 0x2C, 0x3B, 0x5C].contains(codeUnit)) {
        throw FormatException(
